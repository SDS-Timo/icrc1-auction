...
export const usePrincipalToSubaccount = (p: Principal) => {
  const { auction } = useAuction()
  return useQuery('subaccount_' + p.toText(), async () =>
    auction.principalToSubaccount(p),
  )
}
  ...
  return useMutation(
      ...
      auction.registerAsset(formObj.principal, BigInt(formObj.minAskVolume)),
    {
      onSuccess: (res, { principal, minAskVolume }) => {
        if ('Err' in res) {
          enqueueSnackbar(`Failed to add ledger: ${JSON.stringify(res.Err)}`, {
            variant: 'error',
          })
        } else {
          queryClient.invalidateQueries('assets')
          enqueueSnackbar(
            `Ledger ${principal.toText()} added. Asset index: ${minAskVolume}`,
            { variant: 'success' },
          )
        }
      },
...
export const useListOrders = (kind: 'ask' | 'bid') => {
  ...
  return useQuery(
    ...
    {
      onError: (err) => {
        enqueueSnackbar(`Failed to fetch ${kind}s: ${err}`, {
          variant: 'error',
        })
      },
    },
...
export const useNotify = () => {
  ...
  const { enqueueSnackbar } = useSnackbar()
  return useMutation(
    (icrc1Ledger: Principal) => auction.icrcX_notify({ token: icrc1Ledger }),
    {
      onSuccess: (res) => {
        if ('Err' in res) {
          enqueueSnackbar(`Failed to deposit: ${JSON.stringify(res.Err)}`, {
            variant: 'error',
          })
        } else {
          queryClient.invalidateQueries('myCredits')
          enqueueSnackbar(`Deposited ${Number(res.Ok)} tokens successfully`, {
            variant: 'success',
          })
        }
      },
  ...
  return useMutation(
      ...
      ),
    {
      onSuccess: (res) => {
        if ('Err' in res) {
          enqueueSnackbar(
            `Failed to place a ${kind}: ${JSON.stringify(res.Err)}`,
            { variant: 'error' },
          )
        } else {
      ...
      onError: (err) => {
        enqueueSnackbar(`Failed to place a ${kind}: ${err}`, {
          variant: 'error',
        })
      },
    },
...
export const useCancelOrder = (kind: 'ask' | 'bid') => {
  ...
  return useMutation(
    (icrc1Ledger: Principal) =>
      kind === 'bid'
        ? auction.cancelBid(icrc1Ledger)
        : auction.cancelAsk(icrc1Ledger),
    {
      onSuccess: (res) => {
        if ('Err' in res) {
          enqueueSnackbar(
            `Failed to cancel the ${kind}: ${JSON.stringify(res.Err)}`,
            { variant: 'error' },
          )
        } else {
          queryClient.invalidateQueries(kind === 'bid' ? 'myBids' : 'myAsks')
      ...
      onError: (err) => {
        enqueueSnackbar(`Failed to cancel the ${kind}: ${err}`, {
          variant: 'error',
        })
      },
    },
  ...
  return useMutation(
      ...
      }),
    {
      onSuccess: (res) => {
        if ('Err' in res) {
          enqueueSnackbar(
            `Failed to withdraw credit: ${JSON.stringify(res.Err)}`,
            { variant: 'error' },
          )
        } else {
          queryClient.invalidateQueries('myCredits')
          enqueueSnackbar(`Credit withdrawn successfully`, {
            variant: 'success',
          })
        }
      },
      onError: (err) => {
        enqueueSnackbar(`Failed to withdraw credit: ${err}`, {
          variant: 'error',
        })
      },
    },
